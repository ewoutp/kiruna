...
Application.prototype._cleanupImages = function(cb) {
	...
	function getServiceForImage(image) {
		return _.find(self.services, function(s) {
	...
	self.docker.listImages({
		...
		_.each(images, function(image) {
			...
			if (s) {
				...
				imageData[s.name].push({ image: image.RepoTags[0], tag: tag });
			}
		});
		...
		var imageNamesToRemove = _.map(_.flatten(_.map(imageData, function(x) { return x; })), function(x) {
			return x.image;
		});
