...
var Service = function(application, name, options) {
	...
	if (self.options.Registry) {
		self.imageName = self.options.Registry + '/' + self.imageName;
	}
	self.containers = [];
	self.state = {};
	if (!self.name) {
 ...
 */
Service.prototype.getRegistration = function() {
	return this.application.getRegistration();
}

/**
 * Gets the full image name of this service.
 * E.g. myrepository:5000/myimage:mytag
 */
Service.prototype.getImageName = function() {
	return this.imageName;
}

/**
 * Gets the tag of this service.
 * E.g. 1.2.3
 */
Service.prototype.getTag = function() {
	return this.options.Tag || 'latest';
}

/**
 * Gets the image name of this service without the tag.
 * E.g. myrepository:5000/myimage
 */
Service.prototype.getImageNameWithoutTag = function() {
	var self = this;
	var result = self.imageName;
	if (self.options.Tag) {
		result = result.substr(0, result.length - (self.options.Tag.length + 1));
	}
	return result;
}
...
/**
 * Should containers running this service register their ports in the registration store?
 */
Service.prototype.shouldRegisterContainers = function() {
...
/**
 * Are all of the containers running this service in the running state and has the settle timeout expired?
 */
Service.prototype.isUp = function() {
	var self = this;
	return (self.containers.length === self.options.Scale) && (self.state.settled) && _.every(self.containers, function(c) {
		return c.isRunning();
	});
}
...
Service.prototype.pullImage = function(cb) {
	var self = this;
	if (!self.isEnabled()) {
		log.verbose('Skip pulling image for disabled server %s', self.name);
		return cb();
	}
	...
	function worker(cb) {
		...
		img.inspect(function(err, data) {
			...
			if (self.options.Registry) {
				opts.registry = self.options.Registry;
				opts.fromImage = self.options.Registry + '/' + opts.fromImage;
			}
			log.info('Pulling image %s...', self.imageName);
			docker.createImage(opts, function(err, stream) {
				if (err) return cb(err);
				stream.on('error', function(err) {
					log.error('Failed to pull %s', self.imageName, err);
					cb(err);
				});
...
Service.prototype.launch = function(cb) {
	var self = this;
	if (!self.isEnabled()) {
		log.verbose('Skipping disabled service %s', self.name);
		return self.stop(cb);
	}
...
/**
 * Mark that this service is about to stop.
 */
Service.prototype.setStoppingState = function() {
	var self = this;
	...
	container.inspect(function(err, data) {
		if (!err) {
			...
			if (data.State && data.State.Running) {
				...
				return self._registerContainer(container, index, cb);
			}
		}
		return cb();
	});
...
Service.prototype._startContainer = function(index, cb) {
	...
	function createAndStartContainer(cb) {
		...
		if (self.options.Dependencies && (self.options.NetworkMode !== 'host')) {
			startOpts.Links = [];
		...
		// Setup network mode
		if (self.options.NetworkMode) {
			startOpts.NetworkMode = self.options.NetworkMode;
		}
		...
		self.docker.createContainer(createOpts, function(err, container) {
			...
			container.start(startOpts, function(err) {
				...
				self._registerContainer(container, index, cb);
			});
		});
	}
...
Service.prototype._onDependentServiceStarted = function() {
	var self = this;
	if (!self.state.launched || self.state.stopping) return;
 ...
 */
Service.prototype._registerContainer = function(container, index, cb) {
	var self = this;
	self.state.settled = false;
	var c = new Container(self, container, index);
...
/**
 * Gets the number of containers in running state
 */
Service.prototype._getRunningCount = function() {
	var self = this;
	return _.filter(self.containers, function(x) {
		return x.isRunning();
	}).length;
}
...
Service.prototype._onContainerStarted = function(container) {
	...
	var runningCount = self._getRunningCount();
	if (runningCount === 1) {
		// We're the first that was started
		self.emit('started');
	}
	if (runningCount === self.options.Scale) {
		// Wait for the settle timeout and then cleanup
		log.verbose('Waiting for settle timeout');
		setTimeout(function() {
			// Still up?
			if (self._getRunningCount() !== self.options.Scale) return;
